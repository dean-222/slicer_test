...
- v0-14.1.0 (2026-06-23)
  - 뼈 조각 좌표 표시에 단순 로컬 변위를 보여주던 방식에서 실제 3D 공간 상의 현재 월드 중심 좌표 및 월드 회전 각도로 실시간 동기화 출력하도록 개선 (Y 증가, Z 초기화)
  - 4자리 버전 번호 체계를 전역 룰에 부합하는 3자리 v0-X.Y.Z 형식으로 일괄 교정 및 역산 정리 (Y 증가)
- v0-14.0.0 (2026-06-23)
...
- v0-10.1.0 (2026-06-23)
  - 팝업 다이얼로그 기본 세로 높이를 700에서 830으로 확장하여 세그먼트 에디터 Threshold 이펙트의 짤림 현상 개선 (Z 증가)
- v0-10.0.0 (2026-06-23)
  - DICOM DB 로드 시 모듈 전환 없이 독립된 팝업 창으로 띄우도록 BrowserWindowType 강제 설정 적용 (X 증가, Y/Z 초기화)
  - 4번 세그멘테이션 에디터에서 신규 세그멘테이션 생성 시 조각(Segment) 추가(Add)가 차단되는 버그 수정 (sourceVolumeNode 유실 대응 재연동 및 상호 동기화 구현) (Z 증가 -> 룰 3번에 의해 기능변경 X 증가로 통합)
- v0-9.0.0 (2026-06-23)
  - 7. Fragment Manager에 생성된 모델 및 가이드를 일괄 삭제하는 일괄 청소 기능(clearModelsButton / onClearModelsClicked) 추가 (Z 증가)
